        ...
        <div className="one-monster-data row">
          ...
          <div className="col-10">
            ...
            <div className="word-arrangement pt-4">
              ...
              <div>
                <div className="text-xs"><small>原有素材：</small>{click.item.map((v,i)=>(<span key={i} className="font-s px-1">{v}</span>))}</div>
                <div className="text-xs"><small>可換素材：</small>{click.attach.map((v,i)=>(<span key={i} className="font-s px-1">{v}</span>))}</div>
              </div>
            </div>
          </div>
          </div>}
